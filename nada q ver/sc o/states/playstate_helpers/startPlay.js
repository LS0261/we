...
// Posición inicial de cada personaje (igual que FNF PE)
function startCharacterPos(char, x = 0, y = 0, gfCheck = false) {
  if (!char || !char.curCharacter) return;

  // Si es GF y gfCheck activado
  ...
  if (char.positionArray && Array.isArray(char.positionArray)) {
    char.x += char.positionArray[0] ?? 0;
    char.y += char.positionArray[1] ?? 0;
  }
}
...
export async function startPlay(ps, songName) {
  ...
  // POSICIONES
  // ---------------------------
  const bfPos = json.song?.boyfriendPos ?? [700, 450];
  const dadPos = json.song?.dadPos ?? [100, 450];
  const gfPos = json.song?.gfPos ?? [400, 400];
  ...
  await ps.dad.init();
  startCharacterPos(ps.dad, dadPos[0], dadPos[1], true);
  ...
  ps.updateCamera = () => {
    ...
    ps.camFollowX += (ps.camX - ps.camFollowX) * 0.04 * ps.cameraSpeed;
    ps.camFollowY += (ps.camY - ps.camFollowY) * 0.04 * ps.cameraSpeed;
