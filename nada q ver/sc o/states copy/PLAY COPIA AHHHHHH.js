...
if (bpmSections.length === 0) bpmSections.push({time: 0, bpm: json.song.bpm || 120});
...
let hudZoom = 1;
let hudZoomTarget = 1;
let beatLength = 60000 / 120; // ms por beat, si BPM = 120 (puedes cambiar el 120)
let lastBeatTime = 0;
let beatCount = 0;
...
let songPos = getSongPos(); // anticipar latencia del audio
...
// 🔥 Detección de beat ligeramente adelantada
if (audioInst && (songPos - lastBeatTime) >= (currentBeatLength - 5)) {
...
hudZoomTarget = 1;  
lastBeatTime += currentBeatLength; // mejor acumulativo
