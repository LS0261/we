...
export class SpriteAnim {
  ...
  draw(ctx) {
    if (!this.loaded || !this.image || this.image.width === 0 || this.image.height === 0) return;
    ...
    octx.drawImage(
      this.image,
      x, y, width, height,
      Math.round((-frameX) * s), Math.round((-frameY) * s),
      Math.round(width * s), Math.round(height * s)
    );
    ...
    ctx.translate(this.pos[0] + ox, this.pos[1] + oy);
