...
export class SpriteAnim {
  ...
  // Función de tweening para la rotación
  doTweenAngle(targetAngle, duration) {
    ...
    const update = (timestamp) => {
      ...
      if (progress < 1) {
        this.rotation = startAngle + angleDifference * progress;
  ...
  // Función de tweening para el zoom
  doTweenZoom(targetZoom, duration, easing) {
    ...
    const update = (timestamp) => {
      ...
      const progress = (timestamp - startTime) / duration;
      if (progress < 1) {
        this.zoom = startZoom + zoomDifference * progress;
        requestAnimationFrame(update);
      } else {
