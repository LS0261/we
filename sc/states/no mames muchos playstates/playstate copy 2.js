...
function loop(timestamp) {
  ...
  // --- GAME LAYER ---
  camGame.clear();                 // Limpia el canvas de juego
  // Opcional: cámara puede seguir al personaje o a donde quieras
  // Por ejemplo, centrado en boyfriend
  camGame.follow(boyfriend?.x || 0, boyfriend?.y || 0);

  camGame.begin();                 // Aplica transformaciones (scroll/zoom)

  // Dibuja personajes y escenario en ctx de camGame
  if (boyfriend?.draw) boyfriend.draw(camGame.ctx);
  // aquí dibuja dad, gf, escenario, notas, etc. usando camGame.ctx

  // Dibujar notas, receptores, sprites LUA, rating sprites, etc. (todo con camGame.ctx)
  // Ejemplo básico:
  // drawNotes(camGame.ctx);
  // drawStrums(camGame.ctx);
  // drawLuaSprites(camGame.ctx);

  camGame.end();                   // Restaura contexto original

  // --- HUD LAYER ---
  camHUD.clear();                  // Limpia canvas de HUD
  camHUD.begin();                  // Aplica transformaciones (usualmente sin scroll, solo zoom)

  // Dibuja HUD: barra de vida, score, icons, etc.
  healthBar.drawTo(camHUD.ctx);
  drawProgressBar(camHUD.ctx);
  // Más elementos HUD...
  ...
  if (dad?.update) dad.update(delta);
  if (gf?.update) gf.update(delta);

  // --- LÓGICA DE BEAT, BPM, ETC. ---
  let songPos = getSongPos();

  // BPM dinámico y beat logic (tu código aquí)
  // ...

  // Actualiza hudZoom o animaciones relacionadas
  // ...

  // --- NOTAS Y ESTADOS ---
  // Manejo de notas vencidas, removidas, hold notes, misses, scoring
  // Tu lógica aquí, usando las listas bfNotes, lanesHeld, etc.

  // --- DIBUJAR ICONOS, SPRITES DE PUNTAJE, TEXTO HUD ---
  camHUD.begin();

  // Ejemplo texto HUD:
  camHUD.ctx.fillStyle = "#fff";
  camHUD.ctx.font = "20px Arial";
  camHUD.ctx.textAlign = "center";

  const accuracy = calculateAccuracy();
  const hudTextY = healthBar.y + healthBar.barHeight + 30;
  camHUD.ctx.fillText(
    `Score: ${score} | Misses: ${misses} | Accuracy: ${accuracy.toFixed(2)}%`,
    camHUD.width / 2,
    hudTextY
  );

  camHUD.ctx.textAlign = "start";
