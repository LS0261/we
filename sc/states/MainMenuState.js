...
export default class MainMenuState {
  constructor(game) {
    ...
    this.confirmBlinkTimer = 0;
this.confirmBlinkVisible = true;
this.confirmBlinkInterval = 0.08; // Parpadeo más rápido para confirmación
this.confirmBlinking = false;
this.confirmBlinkDuration = 1.0; // 1 segundo de parpadeo antes de transición
...
handleInput(e){
  ...
  if(e.key==="ArrowDown") {
    if(this.curColumn === 'CENTER') this.changeItem(1);
  }
...
changeColumn(direction){
  ...
  if(direction === 'LEFT'){
    if(this.curColumn === 'CENTER') {
      this.curColumn = 'LEFT';
    } else if(this.curColumn === 'RIGHT') {
      this.curColumn = 'CENTER';
      // Restaurar último seleccionado en CENTER
      this.curSelected = this.lastSelectedCenter;
    }
  }
  else if(direction === 'RIGHT'){
    if(this.curColumn === 'CENTER') {
      this.curColumn = 'RIGHT';
    } else if(this.curColumn === 'LEFT') {
      this.curColumn = 'CENTER';
      this.curSelected = this.lastSelectedCenter;
    }
  }

  // En caso que volvamos a CENTER, usar la última selección guardada
  if(this.curColumn === 'CENTER'){
    if(this.curSelected === null || this.curSelected === undefined){
      this.curSelected = this.lastSelectedCenter;
    }
  }
  ...
  if(selectedItem) selectedItem.play('selected');
  ...
  console.log("Columna cambiada a:", this.curColumn, "Índice centro:", this.curSelected);
}
...
changeItem(change=0){
  ...
  if(this.curColumn === 'CENTER') {
    ...
    this.lastSelectedCenter = this.curSelected;  // Guardar selección actual
  }
  ...
  if(this.scrollMenuSound){
    this.scrollMenuSound.currentTime = 0;
...
loop(timestamp) {
  ...
  if(this.confirmBlinking){
    this.confirmBlinkTimer += delta;
    this.confirmBlinkElapsed += delta;

    if(this.confirmBlinkTimer >= this.confirmBlinkInterval){
      this.confirmBlinkVisible = !this.confirmBlinkVisible;
      this.confirmBlinkTimer = 0;
    }

    // Cuando pasen 1 segundo, lanzar transición y detener parpadeo
    if(this.confirmBlinkElapsed >= this.confirmBlinkDuration){
      this.confirmBlinking = false; // parar parpadeo
      ...
      else if(this.selectedOption === "mods"){
        alert("Aquí iría Mods");
      }
      else if(this.selectedOption === "credits"){
        alert("Aquí irían los Créditos");
      }
      else if(this.selectedOption === "achievements"){
        alert("Aquí irían los Logros");
      }
  ...
  this.menuItems.forEach((item, index) => {
    ...
    if(this.confirmBlinking && this.confirmBlinkVisible && this.curColumn === 'CENTER' && index === this.curSelected){
      item.draw(this.ctx);
    }
    else if(this.confirmBlinking && !this.confirmBlinkVisible && this.curColumn === 'CENTER' && index === this.curSelected){
      // no dibujar para parpadeo
    }
    else {
      item.draw(this.ctx);
    }
  ...
  if(this.leftItem){
    if(this.confirmBlinking && this.curColumn === 'LEFT'){
      if(this.confirmBlinkVisible){
        this.leftItem.draw(this.ctx);
      }
    } else {
      this.leftItem.draw(this.ctx);
    }
  ...
  if(this.rightItem){
    if(this.confirmBlinking && this.curColumn === 'RIGHT'){
      if(this.confirmBlinkVisible){
        this.rightItem.draw(this.ctx);
      }
    } else {
      this.rightItem.draw(this.ctx);
    }
